...
$sampler_id = $_SESSION['user_id'];

if (isset($_POST['mark_all_read'])) {
    $appObj->markAllNotificationsRead($sampler_id);
    header("Location: notifications.php"); 
    exit;
}

// Reusing the generic notifications queries
$notifications = $appObj->getStudentNotifications($sampler_id);
$unreadCount = $appObj->getUnreadNotificationCount($sampler_id);
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>إشعارات ضابط العينات | نظام إدارة الموافقات البحثية</title>
...
<body>
    ...
    <div class="content">
        <h2 class="page-title"><i class="fa-solid fa-bell"></i> إشعارات ضابط العينات</h2>
        <p class="page-subtitle">تنبيهات الأبحاث التي تتطلب حساب حجم العينة</p>
        ...
        <?php if (empty($notifications)): ?>
            <div class="empty-state">
                ...
                <p style="font-size:0.9rem;">ستظهر هنا أي تنبيهات بشأن الأبحاث الجديدة.</p>
            </div>
        <?php else: ?>
