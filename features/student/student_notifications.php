...
if (isset($_POST['mark_all_read'])) {
    $appObj->markAllNotificationsRead($student_id);
    header("Location: student_notifications.php"); exit;
}

$notifications = $appObj->getStudentNotifications($student_id);
$unreadCount = $appObj->getUnreadNotificationCount($student_id);
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .notif-icon.type-warning{background:linear-gradient(135deg,#f39c12,#f1c40f)}
        .notif-icon.type-success{background:linear-gradient(135deg,#27ae60,#2ecc71)}
        .notif-icon.type-danger{background:linear-gradient(135deg,#e74c3c,#ec7063)}
...
<body>
    ...
    <div class="content">
        <h2 class="page-title"><i class="fa-solid fa-bell"></i> الإشعارات</h2>
        <p class="page-subtitle">جميع الإشعارات والتنبيهات الخاصة بأبحاثك</p>

        <div class="notif-header">
            <span class="notif-count"><i class="fa-solid fa-bell"></i> إجمالي: <?= count($notifications) ?> <?php if ($unreadCount > 0): ?><span class="badge"><?= $unreadCount ?> جديد</span><?php endif; ?></span>
            <?php if ($unreadCount > 0): ?>
                <form method="POST" style="margin:0"><button type="submit" name="mark_all_read" class="btn-mark-all"><i class="fa-solid fa-check-double"></i> تعيين الكل كمقروء</button></form>
            <?php endif; ?>
        </div>

        <?php if (empty($notifications)): ?>
            <div class="empty-state"><i class="fa-solid fa-bell-slash"></i><p style="font-weight:700;font-size:1.05rem">لا توجد إشعارات</p><p style="font-size:0.9rem">ستظهر هنا أي تحديثات بشأن أبحاثك.</p></div>
        <?php else: ?>
            <div class="notif-list">
                <?php foreach ($notifications as $notif):
                    $isUnread = !$notif['is_read'];
                    $msg = $notif['message'];
                    if (strpos($msg, 'تعديلات') !== false) { $iconType = 'type-warning'; $icon = 'fa-triangle-exclamation'; }
                    elseif (strpos($msg, 'رفض') !== false) { $iconType = 'type-danger'; $icon = 'fa-circle-xmark'; }
                    elseif (strpos($msg, 'اعتماد') !== false || strpos($msg, 'تهانينا') !== false) { $iconType = 'type-success'; $icon = 'fa-circle-check'; }
                    else { $iconType = 'type-info'; $icon = 'fa-info-circle'; }
                ?>
